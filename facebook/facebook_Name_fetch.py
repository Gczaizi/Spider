...
def geturl(Keywords):
    url = []
    for i in Keywords:
        tmp = "http://54.248.44.79:8888/post/facebook?username=%s&num=50" % i
        url.append(tmp)
    return url
...
if __name__ == "__main__":
	...
	#print Key
        name = Key[i] + ' ' + now
        print name + ' ' + Url[i]
        path = "/home/gaochuan/fetch/facebook/result_Name/%s" % Key[i]
        print path
        if os.path.exists(path) != 1:
            os.mkdir(r"result_Name/%s" % Key[i].decode('utf-8'))
        fw = open("result_Name/%s/%s.json" % (Key[i].decode('utf-8'), now), "a+")

	#fw = open("result_Name/%s.json" % name.decode('utf-8'), "a+")
        fw.write(response)
        fw.close()
